...
// ----- tweetsをデータベースから読み込み -----
// クエリの発行(SQL文の実行)
$sql = "SELECT * FROM tweets WHERE `username` = '{$_COOKIE['twitter_username']}' OR `username` IN (SELECT followed_name FROM follows WHERE follower_name = '{$_COOKIE['twitter_username']}') ORDER BY created DESC;";
$result = mysql_query($sql, $link);
...
<html>
    ...
    <body>
<?php if (!empty($_COOKIE['twitter_username'])): ?>
