...
mysql_query('SET NAMES utf8', $link); // MySQL4.0以下ならコメントアウト
	...
	if (!empty($_POST["dopost"]) && !empty($_POST["name"])) {
		...
		// 投稿内容のデータベースへの書込み
		$sql = "INSERT INTO posts SET name='".$_POST["name"]."', title='".$_POST["title"]."', content='".$_POST["content"]."', pass='".$_POST["pass"]."', postdate=NOW()";
	...
	if (!empty($_POST['delete'])) {
		...
		$sql = "DELETE FROM posts WHERE id=".$_POST["id"]." AND pass='".$_POST["pass"]."' LIMIT 1";
		mysql_query($sql, $link);
	...
	if ( $result ) {
		while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
			...
			echo '<blockquote>'.nl2br($row['content']).'</blockquote>';
			echo '</p><hr />';
		}
	}
